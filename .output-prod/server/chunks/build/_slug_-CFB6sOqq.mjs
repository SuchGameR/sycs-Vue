...
import { u as useInfiniteScroll, U as UserBadges_default, a as UserTitle_default, P as PostAttachments_default } from './useInfiniteScroll-Bd156WHj.mjs';
...
import './useMediaPane-BrRmAswF.mjs';
...
var _slug__vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: me } = ([__temp, __restore] = withAsyncContext(() => useFetch("/api/auth/me", { key: "profile-me" }, "$7VDZM0tSqv")), __temp = await __temp, __restore(), __temp);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) _push(`<p class="text-center text-slate-500 py-8">\u8AAD\u307F\u8FBC\u307F\u4E2D...</p>`);
        ...
        if (unref(profile).user.avatarUrl) _push(`<img${ssrRenderAttr("src", unref(profile).user.avatarUrl)} class="w-20 h-20 rounded-full border-4 border-[#0b0f19] object-cover">`);
        ...
        if (unref(isOwnProfile)) {
          ...
          _push(`<button class="px-3 py-1.5 rounded-lg border border-slate-700 text-sm text-slate-300 hover:bg-slate-800 transition" title="DM\u3092\u9001\u308B">`);
          _push(ssrRenderComponent(_component_Icon, {
          ...
          ssrRenderList(unref(filteredPosts), (post) => {
            ...
            _push(ssrRenderComponent(_component_NuxtLink, {
              ...
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) if (post.user.avatarUrl) _push2(`<img${ssrRenderAttr("src", post.user.avatarUrl)} class="w-10 h-10 rounded-full object-cover"${_scopeId}>`);
                ...
                else return [post.user.avatarUrl ? (openBlock(), createBlock("img", {
                  key: 0,
...
//# sourceMappingURL=_slug_-CFB6sOqq.mjs.map
