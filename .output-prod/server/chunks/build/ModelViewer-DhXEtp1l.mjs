...
import { u as useCustomEmojis, s as searchEmoji, a as shouldJumboEmoji } from './richText-BaJfyDxJ.mjs';
...
var EmojiTextarea_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative w-full min-w-0" }, _attrs))} data-v-f1b5ae5f>`);
      _push(ssrRenderComponent(unref(EditorContent), { editor: unref(editor) }, null, _parent));
      if (!unref(hasContent)) _push(`<div class="absolute top-0 left-0 right-0 pointer-events-none text-sm text-slate-500 select-none overflow-hidden" data-v-f1b5ae5f>${ssrInterpolate(props.placeholder || "")}</div>`);
      else _push(`<!---->`);
      if (unref(open)) {
        _push(`<div class="absolute bottom-full left-0 mb-1 w-full max-w-sm max-h-56 overflow-y-auto bg-slate-900 border border-slate-700 rounded-xl shadow-xl z-[150]" data-v-f1b5ae5f><div class="px-3 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-800 sticky top-0 bg-slate-900" data-v-f1b5ae5f> \u7D75\u6587\u5B57 </div><!--[-->`);
        ssrRenderList(unref(suggestions), (s, i) => {
          _push(`<button type="button" class="${ssrRenderClass([i === unref(activeIdx) ? "bg-indigo-600/30" : "hover:bg-slate-800", "w-full flex items-center gap-2.5 px-3 py-1.5 text-left transition"])}" data-v-f1b5ae5f>`);
          if (s.url) _push(`<img${ssrRenderAttr("src", s.url)}${ssrRenderAttr("alt", ":" + s.name + ":")} class="sycs-emoji sycs-emoji--lg shrink-0" draggable="false" data-v-f1b5ae5f>`);
          else _push(`<span class="text-lg leading-none shrink-0" data-v-f1b5ae5f>${ssrInterpolate(s.char)}</span>`);
          _push(`<span class="text-xs text-slate-400 truncate" data-v-f1b5ae5f>:${ssrInterpolate(s.name)}:</span>`);
          if (s.url) _push(`<span class="ml-auto text-[10px] text-slate-600 shrink-0" data-v-f1b5ae5f>\u30AB\u30B9\u30BF\u30E0</span>`);
          else _push(`<!---->`);
          _push(`</button>`);
        });
        _push(`<!--]--></div>`);
...
var EmojiTextarea_default = /* @__PURE__ */ Object.assign(_plugin_vue_export_helper_default(EmojiTextarea_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-f1b5ae5f"]]), { __name: "EmojiTextarea" });
...
export { EmojiTextarea_default as E, ModelViewer_default as M };
//# sourceMappingURL=ModelViewer-DhXEtp1l.mjs.map
