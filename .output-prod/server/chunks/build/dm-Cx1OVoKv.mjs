...
var index_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: me } = ([__temp, __restore] = withAsyncContext(() => useFetch("/api/auth/me", { key: "dm-me" }, "$y6CbIEZ_d0")), __temp = await __temp, __restore(), __temp);
          ...
          _push(ssrRenderComponent(_component_NuxtLink, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              ...
              if (_push2) {
                ...
                if (("avatarSrc" in _ctx ? _ctx.avatarSrc : unref(avatarSrc))((_a = otherMembers(ch)[0]) == null ? void 0 : _a.avatarUrl)) _push2(`<img${ssrRenderAttr("src", ("avatarSrc" in _ctx ? _ctx.avatarSrc : unref(avatarSrc))((_b = otherMembers(ch)[0]) == null ? void 0 : _b.avatarUrl))} class="w-full h-full object-cover"${_scopeId}>`);
                else _push2(`<!--[-->${ssrInterpolate(((_d = (_c = otherMembers(ch)[0]) == null ? void 0 : _c.displayName) == null ? void 0 : _d.charAt(0)) || "?")}<!--]-->`);
                _push2(`</div><div class="min-w-0 flex-1"${_scopeId}><div class="flex items-center justify-between gap-2"${_scopeId}><p class="text-sm font-bold text-white truncate"${_scopeId}>${ssrInterpolate(otherMembers(ch).map((m) => m.displayName).join(", ") || "\u4E0D\u660E")} <span class="text-xs font-normal text-slate-500"${_scopeId}>@${ssrInterpolate(otherMembers(ch).map((m) => m.username).join(", @") || "?")}</span></p><span class="flex items-center gap-1.5 shrink-0"${_scopeId}>`);
                ...
                if ((_e = ch.lastMessage) == null ? void 0 : _e.createdAt) _push2(`<span class="text-[11px] text-slate-600"${_scopeId}>${ssrInterpolate(timeAgo(ch.lastMessage.createdAt))}</span>`);
                else _push2(`<!---->`);
                _push2(`</span></div>`);
                if (ch.lastMessage) {
                  _push2(`<p class="text-xs text-slate-400 truncate"${_scopeId}><span class="${ssrRenderClass(unref(unread).hasDmUnread(ch.id) ? "text-slate-200" : "text-slate-300")}"${_scopeId}>${ssrInterpolate((_f = ch.lastMessage.sender) == null ? void 0 : _f.displayName)}`);
                ...
                if ((_g = otherMembers(ch)[0]) == null ? void 0 : _g.statusMessage) _push2(`<p class="text-[11px] text-emerald-400/80 truncate flex items-center gap-1 mt-0.5"${_scopeId}><span class="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0 inline-block"${_scopeId}></span>${ssrInterpolate(otherMembers(ch)[0].statusMessage)}</p>`);
                else _push2(`<!---->`);
                _push2(`</div>`);
                ...
                ch.lastMessage ? (openBlock(), createBlock("p", {
                ...
                }, [createVNode("span", { class: unref(unread).hasDmUnread(ch.id) ? "text-slate-200" : "text-slate-300" }, [
                  ...
                  createTextVNode(": ")
                ], 2), createTextVNode(toDisplayString(ch.lastMessage.content), 1)])) : (openBlock(), createBlock("p", {
                  key: 1,
...
//# sourceMappingURL=dm-Cx1OVoKv.mjs.map
